                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form class="custom-validation" action="{{ url('admin/category-update/'.$category->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="">Kategori Dil</label>
                                            <select name="lang" class="form-select" aria-label="categori select languages" required>
                                                @php
                                                    $selectedLang = old('lang') ?? $category->lang ?? '';
                                                @endphp
                                                <option disabled value="">Dil Seçiniz</option>
                                                <option value="English" {{ $selectedLang == 'English' ? 'selected' : '' }}>English</option>
                                                <option value="Turkish" {{ $selectedLang == 'Turkish' ? 'selected' : '' }}>Turkish</option>
                                                <option value="Russian" {{ $selectedLang == 'Russian' ? 'selected' : '' }}>Russian</option>
                                                <option value="Deutsch" {{ $selectedLang == 'Deutsch' ? 'selected' : '' }}>Deutsch</option>
                                            </select>                                            
                                        </div>
                                        <div class="mb-3">
